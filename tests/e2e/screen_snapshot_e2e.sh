...
BIN_PATH="${ROOT_DIR}/tmp/do-ai-screen-e2e"
PORT="${DO_AI_E2E_PORT:-19788}"
TOKEN="doai-relay-v1-9f8e7d6c5b4a3928171605ffeeddccbbaa99887766554433221100aabbccddeeff"
BASE_URL="http://127.0.0.1:${PORT}"
SESSION_NAME="screen-e2e-session"
MARKER="SCREEN_E2E_LINE_$(date +%s)"
WORK_DIR="$(mktemp -d)"
...
echo "[SCREEN-E2E] build do-ai"
cd "${ROOT_DIR}"
go build -trimpath -ldflags "-s -w" -o "${BIN_PATH}" ./src

echo "[SCREEN-E2E] start relay @ ${BASE_URL}"
...
echo "[SCREEN-E2E] start do-ai session (cat)"
...
if [[ -z "${SESSION_ID}" ]]; then
  echo "[SCREEN-E2E] ERROR: session not found"
  curl -fsS "${BASE_URL}/api/v1/sessions" -H "X-Relay-Token: ${TOKEN}" || true
  cat "${WORK_DIR}/session.log" || true
  exit 1
fi

echo "[SCREEN-E2E] session_id=${SESSION_ID}"

curl -fsS -X POST "${BASE_URL}/api/v1/control/send" \
  -H "X-Relay-Token: ${TOKEN}" \
  -H 'Content-Type: application/json' \
  --data "{\"session_id\":\"${SESSION_ID}\",\"input\":\"${MARKER}\",\"submit\":true,\"source\":\"screen-e2e\"}" \
  >/dev/null

FOUND="0"
for _ in {1..40}; do
  SCREEN_JSON="$(curl -fsS "${BASE_URL}/api/v1/output/screen?session_id=${SESSION_ID}&limit=260" -H "X-Relay-Token: ${TOKEN}" || echo '{}')"
  if jq -e --arg marker "${MARKER}" '(.lines // []) | any(. == $marker)' <<<"${SCREEN_JSON}" >/dev/null; then
    if jq -e '.revision > 0' <<<"${SCREEN_JSON}" >/dev/null; then
      FOUND="1"
      break
    fi
  fi
...
if [[ "${FOUND}" != "1" ]]; then
  echo "[SCREEN-E2E] ERROR: marker not found in screen snapshot"
  echo "${SCREEN_JSON:-{}}"
  cat "${WORK_DIR}/session.log" || true
  exit 1
fi

echo "[SCREEN-E2E] PASS"
